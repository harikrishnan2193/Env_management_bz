...
exports.postUser_roles = async (req, res) => {
  ...
  try {
    ...
    if (existingRole) {
      ...
      res
        .status(200)
        .json({ success: true, message: "User role added successfully" });
    }
  } catch (error) {
...
exports.postNew_admin = async (req, res) => {
  ...
  try {
    ...
    if (!user_id || !role_id || !organization_id) {
      return res
        .status(400)
        .json({ error: "User ID, Role ID, and Organization ID are required." });
    }
